...
## Purpose
Provides the cross-cutting human-in-the-loop gate that intercepts every tool-call attempt from any built-in tool, shows the user what will run, and only allows execution on explicit per-call approval. TBD: expand with additional context as the capability evolves.
