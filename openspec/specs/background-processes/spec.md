...
## Purpose
Provides tools for launching, monitoring, interacting with, and stopping subprocesses that run in the background independently of the tool call that started them, so the model can manage long-lived processes (e.g. dev servers) within a chat session. TBD: expand with additional context as the capability evolves.
